...
- `for_llm/`: LLM이 단일 API 기능을 쉽게 탐색하고 호출할 수 있도록 구성된 기능 단위 샘플 코드
- `for_user/`: 사용자가 실제 투자 및 자동매매 구현에 활용할 수 있도록 상품별로 통합된 API 호출 예제 코드
...
- 아래 카테고리 및 폴더 구조는 for_llm/, for_user/ 폴더 모두 동일하게 적용됩니다.
...
```bash
# 국내주식 샘플 코드 실행 (for_user/domestic_stock/)
...
```bash
# 국내주식 > 주식현재가 시세 샘플 코드 실행 (for_llm/domestic_stock/inquire_price/)
python chk_inquire_price.py
```

for_llm 은 각 기능별로 개별 실행 파일(chk_*.py)이 분리되어 있어, 특정 기능만 테스트하고자 할 때 유용합니다.

### 4.2. 예제 코드 샘플 (for_user)
